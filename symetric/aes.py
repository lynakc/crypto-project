

# ── S-BOX ────
S_BOX = [
    
]

# ── Inverse S-BOX (auto-generated from S_BOX) ────────────────
INV_S_BOX = [0] * 256
for i, v in enumerate(S_BOX):
    INV_S_BOX[v] = i

# ── Round constants ───
RCON = [0x01, 0x02, 0x04, 0x08, 0x10,
        0x20, 0x40, 0x80, 0x1B, 0x36]


# ── Padding (PKCS7) ────
def pad_text(text):
    pad_len = 16 - (len(text) % 16)
    return text + chr(pad_len) * pad_len

def unpad_text(text):
    pad_len = ord(text[-1])
    return text[:-pad_len]


# ── Key input ────
def get_key():
    key = input("Entrer la clé AES (16 caractères): ")
    if len(key) == 16:
        return {"key": key, "rounds": 10}
    print("Taille invalide")
    return get_key()


# ── GF(2^8) helpers ───
def xtime(x):
    if x & 0x80:
        return ((x << 1) ^ 0x1b) & 0xFF
    return (x << 1) & 0xFF

def mul(a, b):
    """Multiplication in GF(2^8)"""
    result = 0
    for _ in range(8):
        if b & 1:
            result ^= a
        high_bit = a & 0x80
        a = (a << 1) & 0xFF
        if high_bit:
            a ^= 0x1b
        b >>= 1
    return result


# ── SubBytes / SubWord ───
def sub_bytes(state):
    return [[S_BOX[state[i][j]] for j in range(4)] for i in range(4)]

def inv_sub_bytes(state):
    return [[INV_S_BOX[state[i][j]] for j in range(4)] for i in range(4)]

def sub_word(word):
    return [S_BOX[b] for b in word]


# ── ShiftRows ─────
def shift_rows(state):
    result = [row[:] for row in state]
    for i in range(4):
        result[i] = state[i][i:] + state[i][:i]
    return result

def inv_shift_rows(state):
    result = [row[:] for row in state]
    result[1] = state[1][-1:] + state[1][:-1]
    result[2] = state[2][-2:] + state[2][:-2]
    result[3] = state[3][-3:] + state[3][:-3]
    return result


# ── MixColumns ────
def mix_single_column(col):
    a = col.copy()
    col[0] = xtime(a[0]) ^ (xtime(a[1]) ^ a[1]) ^ a[2] ^ a[3]
    col[1] = a[0] ^ xtime(a[1]) ^ (xtime(a[2]) ^ a[2]) ^ a[3]
    col[2] = a[0] ^ a[1] ^ xtime(a[2]) ^ (xtime(a[3]) ^ a[3])
    col[3] = (xtime(a[0]) ^ a[0]) ^ a[1] ^ a[2] ^ xtime(a[3])
    return col

def mix_columns(state):
    for j in range(4):
        col = [state[i][j] for i in range(4)]
        col = mix_single_column(col)
        for i in range(4):
            state[i][j] = col[i]
    return state

def inv_mix_single_column(c):
    a0, a1, a2, a3 = c
    return [
        mul(a0,14) ^ mul(a1,11) ^ mul(a2,13) ^ mul(a3, 9),
        mul(a0, 9) ^ mul(a1,14) ^ mul(a2,11) ^ mul(a3,13),
        mul(a0,13) ^ mul(a1, 9) ^ mul(a2,14) ^ mul(a3,11),
        mul(a0,11) ^ mul(a1,13) ^ mul(a2, 9) ^ mul(a3,14),
    ]

def inv_mix_columns(state):
    for j in range(4):
        col = [state[i][j] for i in range(4)]
        col = inv_mix_single_column(col)
        for i in range(4):
            state[i][j] = col[i]
    return state


# ── AddRoundKey ─────
def add_round_key(state, round_key):
    return [[state[i][j] ^ round_key[i][j] for j in range(4)] for i in range(4)]


# ── Matrix / text / hex conversions ──────
def text_to_matrix(text):
    matrix = [[0]*4 for _ in range(4)]
    for i in range(16):
        matrix[i % 4][i // 4] = ord(text[i])
    return matrix

def matrix_to_text(state):
    return "".join(chr(state[i % 4][i // 4]) for i in range(16))

def hex_to_matrix(hex_string):
    bytes_data = [int(hex_string[i:i+2], 16) for i in range(0, 32, 2)]
    matrix = [[0]*4 for _ in range(4)]
    for i in range(16):
        matrix[i % 4][i // 4] = bytes_data[i]
    return matrix

def matrix_to_hex(state):
    return "".join(f"{state[r][c]:02X}" for c in range(4) for r in range(4))


# ── Key expansion ───
def rot_word(word):
    return word[1:] + word[:1]

def g(word, round_index):
    word = rot_word(word)
    word = sub_word(word)
    word[0] ^= RCON[round_index]
    return word

def key_expansion(key):
    key_bytes = [ord(c) for c in key]
    words = [key_bytes[4*i:4*(i+1)] for i in range(4)]
    for i in range(4, 44):
        temp = words[i-1].copy()
        if i % 4 == 0:
            temp = g(temp, (i // 4) - 1)
        words.append([words[i-4][j] ^ temp[j] for j in range(4)])
    return words

def get_round_keys(words):
    round_keys = []
    for i in range(0, 44, 4):
        matrix = [[0]*4 for _ in range(4)]
        for col in range(4):
            for row in range(4):
                matrix[row][col] = words[i+col][row]
        round_keys.append(matrix)
    return round_keys


# ── Single-block encrypt / decrypt helpers ────
def _encrypt_block(state, round_keys, rounds):
    state = add_round_key(state, round_keys[0])
    for i in range(1, rounds):
        state = sub_bytes(state)
        state = shift_rows(state)
        state = mix_columns(state)
        state = add_round_key(state, round_keys[i])
    state = sub_bytes(state)
    state = shift_rows(state)
    state = add_round_key(state, round_keys[rounds])
    return state

def _decrypt_block(state, round_keys, rounds):
    state = add_round_key(state, round_keys[rounds])
    for i in reversed(range(1, rounds)):
        state = inv_shift_rows(state)
        state = inv_sub_bytes(state)
        state = add_round_key(state, round_keys[i])
        state = inv_mix_columns(state)
    state = inv_shift_rows(state)
    state = inv_sub_bytes(state)
    state = add_round_key(state, round_keys[0])
    return state


# ── Public API ──
def encrypt(message, key):
    
    if isinstance(key, dict):
        rounds = key["rounds"]
        key    = key["key"]
    else:
        rounds = 10
    message    = pad_text(message)                         
    words      = key_expansion(key)
    round_keys = get_round_keys(words)

    result = ""
    for i in range(0, len(message), 16):
        block = message[i:i+16]
        state = text_to_matrix(block)
        state = _encrypt_block(state, round_keys, rounds)
        result += matrix_to_hex(state)
    return result


def decrypt(ciphertext, key):
   
    if isinstance(key, dict):
        rounds = key["rounds"]
        key    = key["key"]
    else:
        rounds = 10
    words      = key_expansion(key)
    round_keys = get_round_keys(words)

    result = ""
    for i in range(0, len(ciphertext), 32):               
        block = ciphertext[i:i+32]
        state = hex_to_matrix(block)
        state = _decrypt_block(state, round_keys, rounds)
        result += matrix_to_text(state)

    return unpad_text(result)                             


